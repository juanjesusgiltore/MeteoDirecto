...
@Composable
fun ProbabilidadNieve(peticionTiempo2ViewModel: PeticionTiempo2ViewModel) {
    ...
    tiempo?.let {
        Box(
            ...
            Column {
                ...
                LazyRow {
                    items(it[0].prediccion.dia[0].probNieve) { probab ->
                        ElementoNieve(probab, peticionTiempo2ViewModel)
                ...
                HorizontalDivider(thickness = 2.dp, color = MaterialTheme.colorScheme.outlineVariant)
                LazyRow {
                    items(it[0].prediccion.dia[1].probNieve) { probab ->
                        ElementoNieve(probab, peticionTiempo2ViewModel)
                        Spacer(modifier = Modifier.width(16.dp))
                    }
                ...
                HorizontalDivider(thickness = 2.dp, color = MaterialTheme.colorScheme.outlineVariant)
                LazyRow {
                    items(it[0].prediccion.dia[2].probNieve) { probab ->
                        ElementoNieve(probab, peticionTiempo2ViewModel)
                        Spacer(modifier = Modifier.width(16.dp))
                    }
