...
import com.tfg.meteodirecto.peticion.retrofit.datos.InstanciaRetrofitDatos
import kotlinx.coroutines.Dispatchers
import kotlinx.coroutines.launch

class PeticionDatosViewModel():ViewModel() {
    ...
    fun getDatos(codMun:String) {
        viewModelScope.launch(Dispatchers.IO) {
            try {
                Log.i("Peti0", codMun)
                val response = InstanciaRetrofitDatos.getRetrofitDatos().getPeticion(apiKey, codMun)
